...
from maplibreum.custom import CustomGlobeLayer

# This is the original JavaScript code from the MapLibre example.
JS_CODE = """
...
def test_add_a_custom_layer_with_tiles_to_a_globe():
    """Validate the original JavaScript injection approach."""
    map_ = Map(
    ...
    map_.add_on_load_js("map.addLayer(highlightLayer, 'countries-label');")
    html = map_.render()
    assert "const highlightLayer" in html
...
def test_add_a_custom_layer_with_tiles_to_a_globe_with_python_api():
    ...
    map_ = Map(
        ...
        center=[7.5, 58],
        zoom=2,
        map_options={"hash": False},  # To make it stable for testing
    )
    map_.add_on_load_js("map.setProjection({type: 'globe'});")

    # Use the new CustomGlobeLayer
    highlight_layer = CustomGlobeLayer(id="highlight")
    map_.add_layer(highlight_layer)
    html = map_.render()
    assert "const highlightLayer_highlight" in html
    assert "map.addLayer(highlightLayer_highlight);" in html